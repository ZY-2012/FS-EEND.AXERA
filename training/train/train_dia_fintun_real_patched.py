...
def train(configs, gpus, checkpoint_resume, test_folder=None):
    ...
    if configs["training"]["init_ckpt"]:
        ...
        ckpt_package = {k.replace('dec.attractor_decoder.layers.', 'dec.layers.'): v
                 for k, v in ckpt_package.items()}
    ...
    # Define the trainer（无 profiler）
    trainer = pl.Trainer(
        max_epochs=configs["training"]["max_epochs"],
        callbacks=callbacks,
        accelerator="gpu", devices=[0],  # PL1.8 的 gpus='0' 字符串会掉到 CPU，强制 GPU
    ...
    for c in ckpts:
        ...
        # Lightning ckpt 含 epoch/state_dict 等混合键，只取 state_dict
    ...
    if configs["log"]["save_avg_path"]:
        import os as _os
        _os.makedirs(_os.path.dirname(configs["log"]["save_avg_path"]), exist_ok=True)
        torch.save(test_state, configs["log"]["save_avg_path"])
        print("Saved averaged model to", configs["log"]["save_avg_path"])
    # trainer.test 跳过：dec 的 MHA 在 val_chunk=100000 帧上触发 CUDA kernel 超限，
    # 评测用独立 streaming_infer_dia.py（流式分块，无此问题）
