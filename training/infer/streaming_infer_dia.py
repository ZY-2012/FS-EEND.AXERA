...
def predict(wav_path, configs, test_file, gpus=0, output_rttm=None, compare_with_batch=True):
    ...
    batch_pred = None
    if compare_with_batch or os.environ.get("DUMP_EMB"):
        with torch.no_grad():
            batch_out, batch_emb, _ = model.test([feat], [len(feat)], max_nspks=max_nspks)
            batch_pred = torch.sigmoid(batch_out[0][:, 1:]).detach().cpu().float()
        print(f"[INFO] batch pred shape: {tuple(batch_pred.shape)}")
        # 诊断用：落盘 LS-EEND 自己的帧嵌入，用于与 CAM++ 嵌入对比判别能力
        if os.environ.get("DUMP_EMB") and output_rttm:
            e = batch_emb[0].detach().cpu().float().numpy()
            np.save(os.path.splitext(output_rttm)[0] + ".emb.npy", e.astype(np.float16))
            print(f"[INFO] emb saved: {e.shape}")
    ...
    if output_rttm is not None:
        ...
        # 同时落盘逐帧概率，便于离线扫 threshold/median 而不必重新推理
        np.save(os.path.splitext(output_rttm)[0] + ".prob.npy",
                stream_pred.numpy().astype(np.float16))
